...
TEST(ServerTest, AddRoute) {
  ...
  server.route("/test", sap::http::EMethod::GET, [](const sap::http::Request &req) {
    return sap::http::Response(200, "Test response");
  });
  SUCCEED();
}
...
TEST(ServerTest, RouteHandlerWithJSON) {
  sap::http::Server server;
  server.route("/api/data", sap::http::EMethod::POST,
...
TEST(ServerTest, MultipleRoutes) {
  ...
  server.route("/", sap::http::EMethod::GET, [](const sap::http::Request &) {
    return sap::http::Response(200, "Home");
  });
  server.route("/api/users", sap::http::EMethod::GET,
               [](const sap::http::Request &) {
                 return sap::http::Response(200, R"([{"id": 1, "name": "John"}])");
               });
  server.route("/api/users", sap::http::EMethod::POST,
               [](const sap::http::Request &req) {
                 return sap::http::Response(201, "User created");
               });
  SUCCEED();
}
