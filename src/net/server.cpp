        ...
        while (true) {
            ...
            if (req_result) {
                ...
                if (req.method == EMethod::UNKNOWN) {
                    ...
                    auto matched = match_route(m_Routes, req.method, req.url.path);
                    const Route* best_match = matched.route;
                    // Middleware runs even for unmatched paths so cross-cutting concerns
                    // (CORS preflight, logging) still see them — a matched public_route
                    // is the only case where middleware is bypassed.
